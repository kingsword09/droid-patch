...
if [ -n "$DROID_SEARCH_DEBUG" ]; then
  ${standaloneEnv}${skipLoginEnv}SEARCH_PROXY_PORT=0 SEARCH_PROXY_PORT_FILE="$PORT_FILE" node "$PROXY_SCRIPT" 2>&1 &
else
  ${standaloneEnv}${skipLoginEnv}SEARCH_PROXY_PORT=0 SEARCH_PROXY_PORT_FILE="$PORT_FILE" node "$PROXY_SCRIPT" >/dev/null 2>&1 &
fi
PROXY_PID=$!
...
${standaloneEnv}${skipLoginEnv}set "SEARCH_PROXY_PORT=0"
...
export async function createWebSearchUnifiedFiles(
  ...
  const wrapperCode = IS_WINDOWS
    ? generateWindowsWrapper(droidPath, proxyScriptPath, standalone, skipLogin)
    : generateUnifiedWrapper(droidPath, proxyScriptPath, standalone, skipLogin);
