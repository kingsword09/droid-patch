...
export interface StatuslineOptions {
  /** 88code proxy config path for balance display */
  proxyConfigPath?: string;
}

function generateStatuslineMonitorScript(options?: StatuslineOptions): string {
  const proxyConfigPath = options?.proxyConfigPath ? JSON.stringify(options.proxyConfigPath) : "null";
...
// === 88code Balance Tracking ===
const PROXY_CONFIG_PATH = ${proxyConfigPath};
const CODE88_CACHE_DURATION_MS = 5 * 60 * 1000; // 5 minutes cache
let code88Balances = []; // { label, used, total, key }
let code88LastFetch = 0;

function load88codeAccounts() {
  if (!PROXY_CONFIG_PATH) return [];
  try {
    const content = fs.readFileSync(PROXY_CONFIG_PATH, 'utf8');
    const config = JSON.parse(content);
    const accounts = new Map(); // key -> { label, key }
    
    for (const instance of (config.instances || [])) {
      for (const forward of (instance.forwards || [])) {
        if (!forward.target || !forward.target.includes('88code')) continue;
        const auth = forward.headers?.authorization || forward.headers?.Authorization;
        if (!auth) continue;
        const key = auth.replace(/^Bearer\\s+/i, '').trim();
        if (!key || accounts.has(key)) continue;
        // Use description or name as label
        let label = forward.description || forward.name || 'unknown';
        // Extract short label from description like "88code king keyss" -> "king"
        const match = label.match(/88code\\s+(\\w+)/i);
        if (match) label = match[1];
        accounts.set(key, { label, key });
      }
    }
...
async function fetch88codeBalance(apiKey) {
  const https = require('https');
  return new Promise((resolve) => {
    const postData = JSON.stringify({});
    const req = https.request({
      hostname: 'www.88code.ai',
      port: 443,
      path: '/api/usage',
      method: 'POST',
      headers: {
        ...
        try {
          ...
          if (json.ok && json.data) {
            ...
            resolve({
              ...
              currentCredits: d.currentCredits || 0,
            });
          } else {
            resolve(null);
          }
...
async function refresh88codeBalances() {
  const now = Date.now();
  if (now - code88LastFetch < CODE88_CACHE_DURATION_MS && code88Balances.length > 0) {
    return; // Use cached data
  }
  const accounts = load88codeAccounts();
  ...
  for (const acc of accounts) {
    const balance = await fetch88codeBalance(acc.key);
    if (balance) {
      const used = (balance.creditLimit - balance.currentCredits).toFixed(1);
      const total = balance.creditLimit.toFixed(0);
      results.push({ label: acc.label, used, total });
    }
  }
  code88Balances = results;
  code88LastFetch = now;
}

function format88codeBalances() {
  if (code88Balances.length === 0) return '';
  return code88Balances.map(b => b.label + ':$' + b.used + '/$' + b.total).join(' ');
}
...
function buildLine(params) {
  ...
  const code88Part = format88codeBalances();
  ...
  const s88code = seg(39, 15, code88Part); // cyan for 88code balances
  ...
  return [sModel, sProvider, sCtx, sUsage, s88code, sBranch, sDiff, sCwd, sExtra].filter(Boolean).join('');
}
...
async function main() {
  ...
  // Initial 88code balance fetch (async, non-blocking)
  if (PROXY_CONFIG_PATH) {
    refresh88codeBalances().then(() => renderNow()).catch(() => {});
    // Refresh 88code balances every 5 minutes
    setInterval(() => {
      refresh88codeBalances().then(() => renderNow()).catch(() => {});
    }, CODE88_CACHE_DURATION_MS).unref();
  }
...
export async function createStatuslineFiles(
  ...
  await writeFile(monitorScriptPath, generateStatuslineMonitorScript(options));
